...
inline float matmul_row(
    ...
    for (uint j=lane*4; j<d_in; j+=32*4) {
        ...
        vec<WeightT,4> w = vec<WeightT,4>(
            ...
            weight[row_idx*d_in+j+1], 
            weight[row_idx*d_in+j+2],
            weight[row_idx*d_in+j+3]
        );
...
[[kernel]] void linear_proj(
    constant LinearProjArgs& args [[ buffer(0) ]],
...
[[kernel]] void softmax(
    ...
    device float* head_in = in + head_idx*args.stride;
    device float* head_out = out + head_idx*args.stride;
    ...
    for (uint i=tid; i<args.dim; i+=thrs_per_thrgp) {
        ...
        float exp_val = exp(head_in[i]-max_val);
        head_out[i] = exp_val;
        sum += exp_val;
    }
...
template [[host_name("linear_proj_bf16")]] [[kernel]] void linear_proj<bfloat>(constant LinearProjArgs&, const device bfloat*, const device float*, device float*, uint, uint);